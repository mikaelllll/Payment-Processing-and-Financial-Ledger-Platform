...
export function LedgerDrawer({ payment, role, onClose }: { payment: Payment; role: Role; onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="drawer">
    <header><div><span className="eyebrow">Immutable accounting evidence</span><h2>{payment.id}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close ledger details"><X/></button></header>
